...
router.post("/api/channels/max-personal/create", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (existingAccounts.length >= FREE_MAX_PERSONAL_ACCOUNTS) {
      // Accounts beyond the free limit require paid extra slots (1 slot = 1 extra account)
      ...
      const paidSlots = extraStatus.extraSlots ?? 0;
      const maxAllowed = FREE_MAX_PERSONAL_ACCOUNTS + paidSlots;

      if (existingAccounts.length >= maxAllowed) {
        return res.status(402).json({
          error: `Для создания более ${maxAllowed} аккаунтов необходимо приобрести дополнительный слот`,
