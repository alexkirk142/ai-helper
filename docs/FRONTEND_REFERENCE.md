...
Переменные определены в `:root` (light) и `.dark` (dark mode).

**Основные переменные:**

| Переменная | Light (hsl) | Назначение |
|-----------|------------|-----------|
| `--background` | 210 5% 98% | Фон страницы |
| `--foreground` | 210 6% 12% | Основной текст |
| `--primary` | 210 78% 48% | Акцентный синий |
| `--card` | 210 5% 96% | Фон карточек |
| `--sidebar` | 210 5% 94% | Фон sidebar |
| `--destructive` | 0 72% 45% | Красный (ошибки) |
| `--muted` | 210 8% 88% | Приглушённые элементы |

**Tailwind конфиг (`tailwind.config.ts`):**
- `darkMode: ["class"]` — через CSS class
- `content`: `./client/index.html`, `./client/src/**/*.{js,jsx,ts,tsx}`
- Кастомные `borderRadius`: lg=9px, md=6px, sm=3px
- Все shadcn цвета подключены через CSS переменные с alpha-value
